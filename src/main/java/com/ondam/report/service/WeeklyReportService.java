    ...
    private String buildMetricsDescription(
        ...
        for (Map.Entry<String, WeeklyReportResponse.MetricDetail> entry : metrics.entrySet()) {
            ...
            if (thisWeekDaily != null && !thisWeekDaily.isEmpty()) {
                ...
                double minValue = thisWeekDaily.stream().mapToDouble(Double::doubleValue).min().orElse(0);
                List<String> lowestDays = new ArrayList<>();
                for (int i = 0; i < thisWeekDaily.size() && i < 7; i++) {
                    if (thisWeekDaily.get(i) == minValue) {
                        lowestDays.add(dayLabels[i]);
                    }
                }
                sb.append("(가장 낮은 요일: ").append(String.join(", ", lowestDays)).append(", 최저값: ").append(minValue).append(") ");
            }
    ...
    private Map<String, String> generateAllComments(
        ...
        try {
            ...
            아래 JSON 형식으로만 답변하세요. 다른 설명 없이 JSON만 출력하세요.
            {
              ...
              "conditionComment": "다음 두 가지를 각각 한 문장씩, 총 두 문장으로 작성하세요. 1) 데이터에 표시된 '가장 낮은 요일'을 그대로 사용해서 설명하세요. 직접 계산하지 말고 주어진 정보를 그대로 인용하세요. '점수가 낮았어요' 대신 '컨디션이 안 좋으셨어요'처럼 상태를 표현하는 말로 쓰세요. 최저값이 2.5 이상이면 낮은 날을 언급하지 말고 '~요일 모두 좋은 컨디션을 유지하셨어요'처럼 칭찬하세요. 2) 이번주 평균이 2.5 이상이면 '대체로 양호했어요' 같은 긍정적 언급을 먼저 하고, 지난주 대비 변화를 덧붙이세요. 이번주 평균이 2.5 미만이면 지난주 대비를 '좋아졌어요', '비슷해요', '나빠졌어요' 중 하나로 짧게 표현하세요. 절대로 숫자나 소수점 수치를 언급하지 마세요.",
              "sleepComment": "다음 두 가지를 각각 한 문장씩, 총 두 문장으로 작성하세요. 1) 데이터에 표시된 '가장 낮은 요일'을 그대로 사용해서 설명하세요. 직접 계산하지 말고 주어진 정보를 그대로 인용하세요. '푹 주무셨어요', '잠이 부족하셨어요'처럼 상태를 표현하는 말로 쓰세요. 최저값이 2.5 이상이면 낮은 날을 언급하지 말고 '~요일 모두 충분히 주무셨어요'처럼 칭찬하세요. 2) 이번주 평균이 2.5 이상이면 '대체로 수면은 양호했어요' 같은 긍정적 언급을 먼저 하고, 지난주 대비 변화를 덧붙이세요. 이번주 평균이 2.5 미만이면 지난주 대비를 '좋아졌어요', '비슷해요', '나빠졌어요' 중 하나로 짧게 표현하세요. 절대로 숫자나 소수점 수치를 언급하지 마세요.",
              "mealComment": "다음 두 가지를 각각 한 문장씩, 총 두 문장으로 작성하세요. 1) 데이터에 표시된 '가장 낮은 요일'을 그대로 사용해서 설명하세요. 직접 계산하지 말고 주어진 정보를 그대로 인용하세요. '잘 챙기셨어요', '많이 거르셨어요'처럼 상태를 표현하는 말로 쓰세요. 최저값이 2.5 이상이면 낮은 날을 언급하지 말고 '~요일 모두 식사를 잘 챙기셨어요'처럼 칭찬하세요. 2) 이번주 평균이 2.5 이상이면 '대체로 식사는 양호했어요' 같은 긍정적 언급을 먼저 하고, 지난주 대비 변화를 덧붙이세요. 이번주 평균이 2.5 미만이면 지난주 대비를 '좋아졌어요', '비슷해요', '나빠졌어요' 중 하나로 짧게 표현하세요. 절대로 숫자나 소수점 수치를 언급하지 마세요.",
              "activityComment": "다음 두 가지를 각각 한 문장씩, 총 두 문장으로 작성하세요. 1) 데이터에 표시된 '가장 낮은 요일'을 그대로 사용해서 설명하세요. 직접 계산하지 말고 주어진 정보를 그대로 인용하세요. '활동량이 많으셨어요', '많이 못 움직이셨어요'처럼 상태를 표현하는 말로 쓰세요. 최저값이 2.5 이상이면 낮은 날을 언급하지 말고 '~요일 모두 활발하게 움직이셨어요'처럼 칭찬하세요. 2) 이번주 평균이 2.5 이상이면 '대체로 활동량은 양호했어요' 같은 긍정적 언급을 먼저 하고, 지난주 대비 변화를 덧붙이세요. 이번주 평균이 2.5 미만이면 지난주 대비를 '좋아졌어요', '비슷해요', '나빠졌어요' 중 하나로 짧게 표현하세요. 절대로 숫자나 소수점 수치를 언급하지 마세요.",
